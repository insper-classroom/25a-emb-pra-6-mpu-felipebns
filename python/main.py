...
def move_mouse(x_value, y_value):
    ...
    pyautogui.moveRel(x_value, y_value, duration=0.05, tween=pyautogui.easeInOutQuad)
...
def controle(ser):
    while True:
        ...
        delta_x, delta_y, end_byte = pacote

        if end_byte != 0xFF:
            continue  # ignora pacotes inválidos

        # Verifica se é o pacote especial de clique
        if delta_x == 0xFE and delta_y == 0xFE:
            print("Clique detectado!")
            pyautogui.click()
            continue

        # Converte os deltas de volta para inteiros com sinal
        x = delta_x - 256 if delta_x > 127 else delta_x
        y = delta_y - 256 if delta_y > 127 else delta_y
